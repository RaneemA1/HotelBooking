...
struct MakkahHotel: Codable , Identifiable{
    let id: UUID?
    let en_city: String
    let ar_city: String
...
class MakkahViewModel:  ObservableObject {
    @Published var makkahHotels: [MakkahHotel] = []
    ...
    func fetchHotel() {
        ...
        let url = URL(string: "https://odqczqchacloconwfpcb.supabase.co/rest/v1/Makkah?select=*")!
        ...
        let task = session.dataTask(with: request) { data, response, error in
            ...
            if let data = data {
                do {
                    ...
                    let makkahHotels = try decoder.decode([MakkahHotel].self, from: data)
                      
                 
                    DispatchQueue.main.async {
                        self.makkahHotels = makkahHotels
                    }
                } catch {
